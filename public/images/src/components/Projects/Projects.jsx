...
const projectSpan = width => {
  const value = Number(width) || 200
  if (value <= 150) return 4
  if (value <= 230) return 6
  return 8
}
...
const ProjectCard = memo(function ProjectCard({ project, onClick }) {
  ...
  return (
    <a
      ...
      href={href}
      style={{
        '--project-col-span': project.gridSpan || projectSpan(project.gridWidth),
        '--project-row-span': project.rowSpan || 1,
        '--project-ratio': project.ratio || '1 / 1',
      }}
...
export default function Projects() {
  ...
  const pointerDown = useCallback(event => {
    ...
    const marquee = marqueeRef.current
    if (!marquee) return
    dragRef.current = { dragging: true, startX: event.clientX, startScrollLeft: marquee.scrollLeft, moved: 0, pointerId: event.pointerId }
  ...
  const pointerMove = useCallback(event => {
    ...
    if (drag.moved > 5) draggedRef.current = true
          ...
          {groups.map(group => (
            <div className="project-group" key={group}>
              {projects.map(project => (
                <ProjectCard key={`${group}-${project.slug}`} project={project} onClick={handleProjectClick} />
              ))}
            </div>
          ))}
        </div>
